...
const callback = (entries) => {
  entries.forEach((entry) => {
    ...
    if (entry.isIntersecting) {
      doc
        .querySelector(`[data-scroll-id="${sectionId}"]`)
        .classList.add("fixed-navbar__link--active");
      doc.getElementById(sectionId).classList.add("section-active");
    } else {
      doc
        .querySelector(`[data-scroll-id="${sectionId}"]`)
        .classList.remove("fixed-navbar__link--active");
...
const getTechnologies = () => {
  ...
  projectsArticles.forEach((project_article) => {
    ...
    if (imgAlt === tech.altRef) {
      const technologiesParagraph = tech.technologies.map(
        (tech) => `<li>${tech}</li>`
      );

      const sectionTechnologies = project_article.querySelector(
...
const keyDownMethod = (input) => {
  ...
  return (!reg.test(input.value) && input.value !== "") ||
